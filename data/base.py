...
def get_data(args, logger):
    ...
    logger.info('In-distribution data prepration...')
    ...
    data_args = {
        'data_path': data_path,
        'train_data_index': dev_data_index,
    ...
    video_feats_data_args = {
        ...
        'train_data_path': train_data_index,
        'dev_data_index': dev_data_index,
        'test_data_index': test_data_index,
    }
    ...
    test_data = MMDataset(test_label_ids, text_data['test'], video_feats_data['test'], audio_feats_data['test'], cons_text_feats['test'], condition_idx['testrain'])
    ...
    with open(read_file_path, 'r') as f:
        ...
        for i, line in enumerate(data):
            ...
            if args.dataset in ['MIntRec']:
                ...
                index = '_'.join([line[0], line[1]])   # MELD dataset에선 데이터의 구조가 데이터셋-라벨의 이중 구조로 되어있기 때문에 index또한 두 pair만 필요함
                indexes.append(index)
