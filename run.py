...
from utils.functions import set_torch_seed, save_results, set_output_paths
...
def parse_arguments():
    ...
    parser.add_argument('--logger_name', type=str, default='Multomodal Intent Recognition', help="Logger name for multimodal intent recognition.")
    ...
    parser.add_arguement('--save_pred', type=bool, default=False, help="Logger directory.")
...
def set_logger(args):
    ...
    ch = logging.StreamHadler()
    ch.setLevel(logging.INFO)
    ch_formatter = logging.Formatter('%(message)')
...
def work(args, data, logger, debug_args=None, ind_args=None):
    ...
    if args.save_results:
        logger.info('Results are saved in %s', str(os.path.join(args.resutls_path, args.results_file_name)))
...
def run(args, data, logger, ind_args = None):
    ...
    if __name__ == '__main__':
        ...
        logger = set_logger(args)  # 왜 똑같은 걸 또 했을까?
        logger.info("="*38+" Params "+"="*30)
        for k in args.keys():
